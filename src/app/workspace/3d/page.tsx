...
import {
  ...
  Maximize, Home, Eye, Footprints, Building2, Ruler,
} from "lucide-react";
...
function BuildingModel({ rooms, showLabels }: { rooms: RoomData[]; showLabels: boolean }) {
  const group = React.useRef<THREE.Group>(null);
  const wallHeight = 10;
  const wallThickness = 0.5;
  const roofColor = "#c4a882";
...
function Workspace3DContent() {
  ...
  const [rooms] = React.useState<RoomData[]>(() => {
    const roomsParam = searchParams.get("rooms") || searchParams.get("project");
    if (roomsParam) {
      try {
  ...
  const defaultCamPos: [number, number, number] = rooms.length > 0
    ? [Math.max(25, rooms.length * 5), 15, 20]
    : [20, 15, 20];
  ...
  return (
    <div className={`h-screen w-full flex flex-col overflow-hidden transition-colors duration-1000 ${dayMode ? 'bg-sky-100' : 'bg-slate-950'}`}>
      <header className="absolute top-0 left-0 w-full p-4 flex items-center justify-between z-20">
        ...
        <div className="flex items-center gap-2">
          <div className={`flex items-center rounded-lg p-1 backdrop-blur-md ${dayMode ? 'bg-white/80' : 'bg-black/50'}`}>
            <a href="/workspace/2d" className={`px-3 py-1 text-xs font-medium transition-colors ${dayMode ? 'text-slate-500 hover:text-slate-800' : 'text-slate-400 hover:text-slate-100'}`}>2D</a>
      ...
      <main className="flex-1 relative">
        <Canvas camera={{ position: walkMode ? [5, 2, 15] : defaultCamPos, fov: walkMode ? 70 : 45 }}>
          {dayMode ? <Sky sunPosition={[100, 20, 100]} /> : <ambientLight intensity={0.2} />}
          ...
          <Float speed={1} rotationIntensity={0.1} floatIntensity={0.1}>
            <BuildingModel rooms={rooms} showLabels={showLabels} />
          </Float>
          <ContactShadows
            position={[0, -0.01, 0]}
            opacity={0.4}
            scale={rooms.length > 0 ? Math.max(30, rooms.length * 8) : 25}
            blur={2}
          />
          <OrbitControls
            enablePan={true}
      ...
      <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2 p-2 rounded-2xl backdrop-blur-md shadow-xl z-20 border transition-colors duration-1000 ${
        ...
        <button onClick={() => setShowLabels(!showLabels)}
          className={`p-3 rounded-xl transition-colors ${showLabels ? 'bg-primary/10 text-primary' : 'hover:bg-slate-100 dark:hover:bg-slate-800 dark:text-slate-300'}`}
          title="Toggle Labels">
          <Eye className="w-5 h-5" />
        </button>
        <div className="w-px h-8 bg-slate-300 dark:bg-slate-700 mx-1" />
        <button className="p-3 rounded-xl transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 dark:text-slate-300"
          title="Reset View" onClick={() => window.location.reload()}>
        ...
        <button className="p-3 rounded-xl transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 dark:text-slate-300"
          title="Fullscreen" onClick={() => document.documentElement.requestFullscreen?.()}>
      ...
      {rooms.length > 0 && (
        <div className="absolute bottom-24 left-4 z-20">
          <div className={`px-3 py-2 rounded-xl backdrop-blur-md shadow-md text-xs font-mono ${dayMode ? 'bg-white/80 text-slate-700' : 'bg-black/60 text-slate-300'}`}>
            <div className="flex items-center gap-2">
              <Building2 className="w-3.5 h-3.5" />
              <span className="font-semibold">{rooms.length} rooms</span>
            </div>
            <div className="mt-1 text-[10px] opacity-60">
              {rooms.reduce((s, r) => s + (r.area_sqft || 0), 0).toFixed(0)} sqft total
            </div>
          </div>
        </div>
      )}
